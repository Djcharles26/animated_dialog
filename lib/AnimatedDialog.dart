...
class _AnimatedDialogState extends State<AnimatedDialog> with SingleTickerProviderStateMixin {
  ...
  Animation transformAnimation, heightAnimation, widthAnimation, opacityAnimation;
  ...
  void initState() { 
    ...
    animationController = AnimationController(duration: this.widget.animationTime?? Duration(milliseconds: 700 ), vsync: this);
    ...
    heightAnimation = Tween(begin: 0.7, end: 1.0).animate(CurvedAnimation(
      parent: animationController, curve: Interval(0.5, 1, curve: Curves.elasticOut)
    ));

    widthAnimation = Tween(begin: 0.3, end: 1).animate(
      CurvedAnimation(
    ...
    opacityAnimation = Tween(begin:0.0, end: 1.0).animate(CurvedAnimation(
      parent: animationController, curve: Interval(0.7, 1.0, curve: Curves.easeIn)
    ));
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder:(ctx, _){
        ...
        print(currentWidth);
        return Transform(
          transform: Matrix4.translationValues(0.0, this.transformAnimation.value * MediaQuery.of(context).size.height,0.0),
